...
parser = argparse.ArgumentParser(
    prog='http-monitor',
    description='Nagios-compatible plugin to check a resource availability over HTTP/HTTPS. By default the resource '
                'is considered to be in a OK status if the HTTP status code is 200 and in CRITICAL status '
...
parser.add_argument(
    ...
    default=30,
    type=int,
    help='maximum time (in seconds) to wait for a response, after which the resource is considered in CRITICAL status'
)
parser.add_argument(
    '--redirect-unknown',
    action='store_true',
    help='if enabled a 302 status code will result in UNKNOWN status. If disabled a 302 status code will result in '
         'CRITICAL status'
)
parser.add_argument(
    '--debug',
...
def http_monitor(url, timeout=30, redirect_unknown=True, debug=False):
    ...
    try:
        ...
        if response.status_code == requests.codes.ok:
            ...
            nagios.plugin_exit(nagios.Codes.UNKNOWN, 'redirection detected with status code %d' % response.status_code)
        else:
